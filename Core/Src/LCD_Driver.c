...
void LCD_Draw_X(uint16_t X, uint16_t Y)
{
	for (uint16_t i = 0; i < 30; i++)
	    {
			// Draw top-left to bottom-right diagonal
			LCD_Draw_Pixel(X + i, Y + i, LCD_COLOR_BLUE);
	        LCD_Draw_Pixel(X + i + 1, Y + i, LCD_COLOR_BLUE);
	        LCD_Draw_Pixel(X + i, Y + i + 1, LCD_COLOR_BLUE);
	        LCD_Draw_Pixel(X + i + 1, Y + i + 1, LCD_COLOR_BLUE);

	        // Draw top-right to bottom-left diagonal//top-right to bottom-left
	        LCD_Draw_Pixel(X + 30 - 1 - i, Y + i, LCD_COLOR_BLUE);
	        LCD_Draw_Pixel(X + 30 - 2 - i, Y + i, LCD_COLOR_BLUE);
	        LCD_Draw_Pixel(X + 30 - 1 - i, Y + i + 1, LCD_COLOR_BLUE);
	        LCD_Draw_Pixel(X + 30 - 2 - i, Y + i + 1, LCD_COLOR_BLUE);
	    }
}

// Function to draw a hollow "O" on the LCD
void LCD_Draw_O(uint16_t Xpos, uint16_t Ypos) {
    uint16_t OuterRad = 16;  // Set the outer radius of the "O"
    uint16_t InnerRad = 14;  // Set the inner radius of the "O"
    uint16_t color = LCD_COLOR_RED;  // Color for the "O"
    uint16_t backgroundColor = LCD_COLOR_WHITE;  // Background color of the LCD

    // Draw the outer circle
    LCD_Draw_Circle_Fill(Xpos, Ypos, OuterRad, color);
